(function(){
    angular.module('ezeidApp').directive('transactionFilter',['$compile',function($compile){
        //var template = '<button class="btn btn-sm btn-warning no-radius" '+
        //'title=\'<h4 class="font-open-sans-light">Filter by'+
        //'   <span class="pull-right glyphicon glyphicon-remove popover-close tiny"></span>'+
        //'   </h4>\''+
        //'data-content=\''+
        //'   <label class="font-open-sans-light text-info">Status Type</label>'+
        //'<div class="form-group form-group-sm"> {{filterList}}'+
        //'    <select class="form-control" ng-model="filterType">'+
        //'    <option ng-repeat="filterStatus in filterList track by $index" ng-init="count = $index" value="{{filterList[$index].TID}}">{{filterList[$index].StatusTitle}}</option>'+
        //'    </select>'+
        //'    </div>'+

        //'   <label class="font-open-sans-light text-info">Action Type</label>'+
        //'<div class="form-group  form-group-sm">'+
        //'    <select class="form-control">'+
        //'    <option value="">All</option>'+
        //'    <option value="">New</option>'+
        //'    <option value="">Accepted</option>'+
        //'    </select>'+
        //'    </div>'+
        //'    \''+
        //'    >'+
        //'    <span class="glyphicon glyphicon-filter"></span> Filters'+
        //'    </button>';

        var popoverHtml = '<label class="font-open-sans-light text-info">Status Type</label>'+
        '<div class="form-group form-group-sm"> {{filterList}}'+
        '    <select class="form-control" ng-model="filterType">'+
        '    <option ng-repeat="filterStatus in filterList track by $index" ng-init="count = $index" value="{{filterList[$index].TID}}">{{filterList[$index].StatusTitle}}</option>'+
        '    </select>'+
        '    </div>'+

        '   <label class="font-open-sans-light text-info">Action Type</label>'+
        '<div class="form-group  form-group-sm">'+
        '    <select class="form-control">'+
        '    <option value="">All</option>'+
        '    <option value="">New</option>'+
        '    <option value="">Accepted</option>'+
        '    </select>'+
        '    </div>';

        var htmlTemplate = '<div>' +
            '<button class="btn btn-sm btn-warning no-radius" ' +
                'title=\'<h4 class="font-open-sans-light">Filter by'+
                '   <span class="pull-right glyphicon glyphicon-remove popover-close tiny"></span>'+
                '   </h4>\'' +

            'data-content=\'' +
            '<label class="font-open-sans-light text-info">Status Type</label>'+
            '<div class="form-group form-group-sm">'+
            '    <select class="form-control" ng-model="filterType">'+
            //'    <option ng-repeat="filterStatus in filterList track by $index" ng-init="count = $index" value="{{filterList[$index].TID}}">{{filterList[$index].StatusTitle}}</option>'+
                + '{{popOverContent}}'
            '    </select>'+
            '    </div>'+

            '   <label class="font-open-sans-light text-info">Action Type</label>'+
            '<div class="form-group  form-group-sm">'+
            '    <select class="form-control">'+
            '    <option value="">All</option>'+
            '    <option value="">New</option>'+
            '    <option value="">Accepted</option>'+
            '    </select>'+
            '    </div>\'' +
            '>'+
            '<span class="glyphicon glyphicon-filter"></span>' +
            ' Filters</button>' +
            '</div>';


        var linker = function(scope,element,attrs){
            //var popoverContent = $('<div/>').html(popoverHtml).contents();

            scope.popOverContent = '';
            for(var i= 0; i < scope.filterList.length; i++){
                scope.popOverContent += '<option value="'+ scope.filterList[i].TID +'">'+ scope.filterList[i].StatusTitle +'</option>';
            }

            //console.log(scope.popOverContent);

            element.html(htmlTemplate).show();
            element.find('button').popover({
                html : true,
                animation : true
            });
            $compile(element.contents())(scope);
        };

        return {
            restrict : "E",
            replace : true,
            //require : '^ngModel',
            link: linker,
            scope: {
                filterList : '=',
                filterType :'=',
                popOverContent : '='
            }
        };

    }]);
})();