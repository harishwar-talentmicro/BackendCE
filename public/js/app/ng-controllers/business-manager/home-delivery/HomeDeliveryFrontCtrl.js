        ...
        function ($rootScope,
            ...
            $scope.resetModalBox = function(){
                $scope.modalBox = {
                    ...
                    tx : {
                        ...
                        folderRule : 0,
                        message : '',
                        messageType : (parseInt($scope.masterUser.HomeDeliveryItemListType) !== NaN) ?
                            parseInt($scope.masterUser.HomeDeliveryItemListType) : 0,
                        latitude : 0,
                        longitude : 0,
                        duration : 0,
                        durationScale : 0,
            ...
            $scope.getLoggedInUserDetails = function(){
                ...
                $http({
                    ...
                    params : {
                        ...
                        /**
                         * Change this variable to $routeParams.ezeone so that now /IND1/homeDelivery path can be supported
                         */
            ...
            $scope.getUserDetails = function(){
                ...
                $http({
                    ...
                    /**
                     * Check whether homeDelivery module visibility is enabled or
                     * not for the ezeid to which homeDelivery enquiry is posted
                     */
                    if (resp && resp != 'null' && resp.length > 0) {
            ...
            $scope.saveTransaction = function(){
                ...
                if($scope.homeDeliveryItemListType > 0){
                    var itemList = [];
                    try{
                    ...
                    for(var ct = 0; ct < itemList.length; ct++){
                        ...
                        if(itemList[ct]['Qty'] && ($scope.homeDeliveryItemListType == 3 || $scope.homeDeliveryItemListType ==4)){
                            msg += ' ('+ itemList[ct]['Qty'] + ')';
                        }
                        if(itemList[ct]['Amount'] && $scope.homeDeliveryItemListType ==4){
            ...
            function calculateCartDetails (){
                ...
                for(var ct = 0; ct < $scope.modalBox.tx.itemList.length; ct++){
                    if($scope.modalBox.tx.itemList[ct]['Qty'] && ($scope.homeDeliveryItemListType == 3 || $scope.homeDeliveryItemListType == 4)){
                    ...
                    if($scope.modalBox.tx.itemList[ct]['Amount'] && $scope.homeDeliveryItemListType == 4){
                        var am = 0.00;
                        var qt = 1;
