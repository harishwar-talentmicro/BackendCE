(function(){
    angular.module('ezeidApp').controller('HomeDeliveryCtrl',[
        '$rootScope',
        '$scope',
        '$http',
        '$q',
        ...
        function (
            ...
            $scope._tempHomeDeliveryItemListType = $rootScope._userInfo.HomeDeliveryItemListType;
            $scope.txSearchTerm = '';

            /**
             * Logged in user cannot use this module as he is not having the required permissions for it
             */
            var moduleIndex = $scope.modules.indexOfWhere('type','home-delivery');
            ...
            $scope.homeDeliveryListConf = listConf[$scope.modules[moduleIndex].listType];
            $scope.homeDeliveryPermissionConf = permissionConf[$scope.modules[moduleIndex].permission];
            ...
            $scope.loadItemList = function(){
                ...
                $http({
                    ...
                    if(resp && resp !== 'null' && resp.length > 0){
                        ...
                        $rootScope._userInfo.HomeDeliveryItemListType = 0;
                        defer.resolve([]);
                    }
                }).error(function(err){
                    defer.reject();
                ...
                $scope.loadFolderRules().then(function(){
                    getSubUserList().then(function(){
                        $scope.loadTxActionTypes().then(function(){
                            ...
                            $scope.$emit('$preLoaderStop');
                            Notification.error({message : 'Unable to load home delivery next actions list', delay : MsgDelay} );
                        });
                    },function(){
                        $scope.$emit('$preLoaderStop');
                    });
            ...
            var calculateTxAmount = function(itemList){
                ...
                if(parseInt($rootScope._userInfo.HomeDeliveryItemListType) > 3){
                    var amount = 0.00;
            ...
            var prepareSaveTransaction = function(editMode){
                ...
                var preparedTx = {
                    ...
                    item_list_type : $rootScope._userInfo.HomeDeliveryItemListType,
                    ...
                    Amount : (parseInt($rootScope._userInfo.HomeDeliveryItemListType) < 4) ?
            ...
            function calculateCartDetails (){
                ...
                for(var ct = 0; ct < $scope.modalBox.tx.itemList.length; ct++){
                    if($scope.modalBox.tx.itemList[ct]['Qty'] &&
                        (parseInt($rootScope._userInfo.HomeDeliveryItemListType) == 3 || parseInt($rootScope._userInfo.HomeDeliveryItemListType) == 4)){
                    ...
                    if($scope.modalBox.tx.itemList[ct]['Amount'] && parseInt($rootScope._userInfo.HomeDeliveryItemListType) == 4){
                        var am = 0.00;
                        var qt = 1;
