    ...
    req.db.query(procQuery, function (err, Result) {
        ...
        if (!err && Result && Result[0] && Result[1] && Result[1][0] && Result[1][0].cvPath != "") {
            try {
                ...
                // facesheet is array
                var faceSheetData = Result[2] && Result[2][0] && Result[2][0].faceSheetData && JSON.parse(Result[2][0].faceSheetData) ? JSON.parse(Result[2][0].faceSheetData) : [];
                // custom Facesheet is html table data
                var customFaceSheet = Result[2] && Result[2][0] && Result[2][0].customFaceSheet ? Result[2][0].customFaceSheet : '';
                console.log('faceSheetData', faceSheetData)
                console.log('customFaceSheet', customFaceSheet)
                ...
                new Promise(function (resolve, reject) {
                    ...
                    var spawn_process = spawn('python', [path.resolve(__dirname, "word.py"), 'https://storage.googleapis.com/ezeone/', orgCVPath, uniqueId, logoFile, clientCVHeader, clientCVFooter, clientCVMaskMobileNo, clientCVMaskEmail, logoAligment, clientCVHeaderAligment, clientCVFooterAligment, faceSheetData, customFaceSheet]);
...
resumeMaskingCtrl.resume_maskingNew = function (req, res, next) {
