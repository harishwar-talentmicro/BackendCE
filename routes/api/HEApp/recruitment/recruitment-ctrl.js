            ...
            if((!err) && tokenResult){
                ...
                zlib.unzip(decryptBuf, function (_, resultDecrypt) {
                    ...
                    if (!validationFlag){
                        ...
                        var procQuery = 'CALL HE_save_referCV_new( ' + procParams.join(',') + ')';
                        console.log(procQuery);
                        req.db.query(procQuery,function(err,results){
                            console.log(results);
                            if(!err && results && results[0] ){
...
recruitmentCtrl.saveProcessUpdateFeedback = function(req,res,next){
    var response = {
        status : false,
        message : "Invalid token",
        data : null,
        ...
        req.st.validateToken(req.query.token,function(err,tokenResult){
            if((!err) && tokenResult){
                ...
                zlib.unzip(decryptBuf, function (_, resultDecrypt) {
                    ...
                    if (!req.body.processUpdateId)
                    {
                        error.processUpdateId = 'Invalid processUpdateId';
                        validationFlag *= false;
                    }
                    ...
                    if (!validationFlag){
                        ...
                        var procParams = [
                            req.st.db.escape(req.query.token),
                            req.st.db.escape(req.body.processUpdateId),
...
recruitmentCtrl.getProcessUpdateFeedback = function(req,res,next){
    var response = {
        status : false,
        message : "Invalid token",
        data : null,
    ...
    if (!validationFlag){
        ...
        req.st.validateToken(req.query.token,function(err,tokenResult){
            if((!err) && tokenResult){
                 ...
                 * @type {string}
                 */
                var procQuery = 'CALL he_get_processUpdateFeedback( ' + procParams.join(',') + ')';
                console.log(procQuery);
